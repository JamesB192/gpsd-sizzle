            ...
            if sniffer.COMMENT_PACKET == ptype:
                ...
                if b"Serial:" in packet:
                    ...
                    try:
                        ...
                        raise TestLoadError(
                            "bad serial-parameter spec in %s"
                            % self.name
                        )
                    ...
                    try:
                        ...
                        raise TestLoadError(
                            "bad Delay-Cookie line in %s" % self.name
                        )
                    self.resplit = True
                ...
                if not packet:
                    raise TestLoadError(
                        "zero-length packet from %s" % self.name
                    )
                self.sentences.append(packet)
...
class FakeGPS(object):
    ...
    def feed(self):
        ...
        line = self.testload.sentences[
            self.index % len(self.testload.sentences)
        ]
        if b"%Delay:" in line:
...
class FakePTY(FakeGPS):
    ...
    def __init__(
        ...
        (iflag, oflag, cflag, lflag, ispeed, ospeed, cc) = (
            termios.tcgetattr(self.slave_fd)
        )
        cc[termios.VMIN] = 1
...
class FakeTCP(FakeGPS):
    ...
    def __init__(self, testload, host, port, progress=lambda x: None):
        """Init class FakeTCP."""
...
class FakeUDP(FakeGPS):
    ...
    def __init__(self, testload, ipaddr, port, progress=lambda x: None):
        """Init FakeUDP."""
...
class SubprogramInstance(object):
    ...
    def spawn_sub(
        self, program, options, background=False, prefix="", env=None
    ):
        ...
        if os.environ.get("GPSD_HOME"):
            for path in os.environ["GPSD_HOME"].split(":"):
                ...
                if os.path.isfile(_spawncmd) and os.access(
                    _spawncmd, os.X_OK
                ):
                    spawncmd = _spawncmd
                    break
        ...
        if not background:
            ...
            if os.WIFSIGNALED(status) or os.WEXITSTATUS(status):
                raise self.ERROR(
                    "%s exited with status %d" % (program, status)
                )
    ...
    def gps_add(self, logfile, speed=19200, pred=None, oneshot=False):
        ...
        if logfile not in self.fakegpslist:
            ...
            if "UDP" == testload.sourcetype or self.udp:
                ...
                newgps = FakePTY(
                    testload, speed=speed, progress=self.progress
                )
            if pred:
            ...
            while self.daemon:
                ...
                if isinstance(chosen, FakeGPS):
                    ...
                    while chosen.waiting():
                        if (
                            not self.daemon
                            or not self.daemon.is_alive()
                        ):
                            raise TestSessionError("daemon died")
                        ret = chosen.read()
                        if 0 > ret:
                            raise TestSessionError(
                                "daemon output stopped"
                            )
                        ...
                        if (
                            "DEVICE" == chosen.data["class"]
                            and 0 == chosen.data["activated"]
